...
path_csv = data_dir / "properties" /"apartments.csv"
path_csv_tien_ich = data_dir / "amenities" / "apartment.csv"
...
for num in range(1, 10):
    ...
    for res in result:
        ...
        if Loai_dat.strip() != 'Căn hộ chung cư':
            continue 
        ...
        dct.setdefault('Tên dự án / tòa nhà', []).append(dct_.get('Tên dự án / tòa nhà', None))
        dct.setdefault('Tình trạng sử dụng', []).append(dct_.get('Tình trạng sử dụng', None))
        dct.setdefault('Địa chỉ chi tiết', []).append(dct_.get('Địa chỉ chi tiết', None))
        dct.setdefault('Hướng ban công', []).append(dct_.get('Hướng ban công', None))
        dct.setdefault('Diện tích tim tường', []).append(dct_.get('Diện tích tim tường', None))
